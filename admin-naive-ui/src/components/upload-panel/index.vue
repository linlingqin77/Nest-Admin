            ...
            <div v-if="show && uploadTasks.length > 0" class="upload-panel" :style="panelStyle">
                ...
                <div class="panel-header">
                    <span class="header-title">
                        上传任务 ({{ completedCount }}/{{ uploadTasks.length }})
                    </span>
                ...
                <n-scrollbar v-if="uploadTasks.length <= 20" :style="{ height: contentHeight }">
                    <div class="task-list">
                        <div v-for="task in uploadTasks" :key="task.id" class="task-item">
                            <UploadTaskItem :task="task" @pause="handlePause(task.id)" @resume="handleResume(task.id)"
                                @cancel="handleCancel(task.id)" @retry="handleRetry(task.id)" />
                        </div>
                    </div>
                </n-scrollbar>

                <!-- 虚拟列表（任务数 > 20 时） -->
                <n-virtual-list v-else :item-size="UPLOAD_ITEM_HEIGHT" :items="uploadTasks"
