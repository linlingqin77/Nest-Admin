<template>
    <div class="upload-task-item" :style="{ height: `${UPLOAD_ITEM_HEIGHT}px` }">
        ...
        <div class="task-content">
            ...
            <!-- 进度条 -->
            <n-progress :percentage="task.progress" :height="3" :show-indicator="false" :status="progressStatus"
            ...
            <div class="task-meta">
                ...
                <span v-if="task.status === 'success'" class="success-text">
                    上传完成
                </span>
                <span v-if="task.status === 'error'" class="error-text">
                    {{ task.error || '上传失败' }}
                </span>
                <span v-if="task.status === 'paused'" class="paused-text">
                    已暂停
                </span>
                <span class="size-info">
                    {{ formatSize(task.uploaded) }} / {{ formatSize(task.fileSize) }}
                </span>
            </div>
        </div>
